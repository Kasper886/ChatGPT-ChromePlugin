...
def save_selected_model(model_name):
    ...
    try:
        logging.info(f"📝 DEBUG: Saving model {model_name}")
        with open(SELECTED_MODEL_FILE, "w") as f:
            f.write(model_name)
            f.flush()
            os.fsync(f.fileno())
        logging.info(f"✅ DEBUG: Model {model_name} saved successfully")
...
async def select_model_menu(message: Message):
    """ Handles /setmodel command by showing a menu """
